...
module.exports.getUser = async function (req, res, next) {
  ...
  try {
    let user = await User.findOne({ username: username }).lean().select('-__v');
...
module.exports.getTargets = async function (req, res, next) {
  let { username } = req.params;
  try {
    let targets = await User.findOne({ username: username })
      .lean()
      .select('-_id targets');
...
module.exports.getMeals = async function (req, res, next) {
  let { username } = req.params;
  try {
    let meals = await User.findOne({ username: username })
      .lean()
      .select('-_id meals');
